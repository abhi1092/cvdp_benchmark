...
class Repository:
    ...
    def log_docker(self, docker : str = "", cmd : str = "", service : str = "", logfile : str = "", 
        ...
        with open(script_path, 'w') as script_file:
            ...
            # Instead, Podman's rootless mode automatically maps the user
            if container_runtime.is_podman():
                ...
                script_file.write(f"  {container_runtime.get_compose_command()} -f {docker} -p {project_name} run --rm -e HOME=/code/rundir --entrypoint bash {cmd} {service}\n")
                script_file.write(f"else\n")
                script_file.write(f"  {container_runtime.get_compose_command()} -f {docker} -p {project_name} run --rm -e HOME=/code/rundir {cmd} {service}\n")
                script_file.write(f"fi\n")
            else:
                script_file.write(f"if [ \"$DEBUG_MODE\" = true ]; then\n")
                script_file.write(f"  echo \"DEBUG MODE: Starting container with bash entrypoint\"\n")
